...
- **待办重构为卡片列表与专注详情页**：任务点击后进入格子内的完整详情页。列表支持更清晰的任务卡片、8 种颜色筛选、最多 10 行文字、拖动排序、截止日期、提醒、重复、附件，以及右键批量操作。
- **随记升级为图文记录工具**：随记采用只保留正文的专注编辑页，支持纸张样式、添加和替换图片、图片预览与复制、固定、创建时间显示、普通与固定列表拖动排序，以及批量固定、纸张、复制和删除。
- **音乐和天气自适应布局**：格子最小尺寸降至 150 x 150。音乐支持封面居中裁剪、紧凑悬浮控制、歌名跑马灯和平滑封面切换，并完全移除频谱功能及相关设置。
- **菜单与多选体验统一**：文件、待办和随记右键命令执行后会立即关闭。文件格子支持多选操作；待办和随记区分拉框多选与普通点击进入详情，避免残留选中状态。
- **动态层级与标题交互修复**：托盘和快捷键能够一致地唤起全部格子，随后把层级交还 Windows 管理。所有格子支持菜单直接重命名、双击标题编辑和更稳定的焦点退出逻辑。
- **设置与重置语义明确**：全局重置保留语言、开机启动和功能格子开关，只恢复偏好设置；功能格子重置增加二次确认，并清除对应数据和设置。新安装与重置默认使用云母、细边框、大圆角、待办/随记分段按钮，并关闭待办底部数量。
- **性能与稳定性**：减少无关格子刷新，完善事件与取消令牌释放，优化音乐会话和设置窗口生命周期，并扩展待办、随记、天气及默认设置的回归测试。
