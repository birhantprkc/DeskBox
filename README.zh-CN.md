...
- **高 DPI 图标清晰度**：文件和快捷方式图标现在提取 256×256（Jumbo）或 48×48（Extra Large），在高 DPI 显示器上与桌面图标一样清晰。
- **Steam 快捷方式图标**：修复 `.url` 快捷方式图标不显示的问题，解析逗号分隔的图标索引并通过注册表和常见路径搜索 Steam。
- **动画性能**：将透明度/缩放动画切换为 GPU Composition KeyFrame 动画和直接 `SetWindowPos` P/Invoke，消除低端 GPU 上的卡顿。
- **黑屏过渡修复**：材质背景在托盘显示/隐藏动画期间保持一致。
- **音乐封面圆角**：修复音乐格子封面圆角不对称的问题。
- **重复启动项**：统一为单一注册表 `Run` 键，自动清理旧快捷方式。
- **胶囊模式默认值**：新安装默认使用悬停自动展开、灵敏悬停响应和舒缓动画。
- **卡死诊断**：新增 UI 线程看门狗，修复 Store 启动的 sync-over-async 死锁。
