...
- 新增完整中文和英文支持，覆盖格子、设置、托盘菜单、弹窗、备注、新用户引导和状态提示。
- 重做新用户引导，在流程中直接展示并配置关键选项，包括拖入后移动/复制、默认收纳路径、文件夹映射和开机自启。
- 优化新用户引导右侧配图和动画，让每一步都更贴合当前说明。
- 修复开机自启后自动打开设置页的问题，现在会静默启动到托盘。
- 优化失效快捷方式处理，双击时调用 Windows 原生的快捷方式修复/删除提示。
- 重做桌面格子的显示/隐藏动画，统一从右往左出现，不再逐个递进，减少闪屏和图标二次动画。
- 修复托盘右键菜单“显示全部组件”没有临时置顶的问题，现在和左键点击托盘图标一致。
- 优化跨格子选择状态、鼠标框选流畅度、映射文件夹刷新、浅色模式文字和背景对比。
